...
class DynamoNixlConnector:
    def __init__(self, vllm_config: VllmConfig, engine_id: str, rank: int):
        ...
        self.num_heads = self.model_config.get_num_kv_heads(self.parallel_config)
    ...
    def register_kv_caches(self, kv_caches: List[torch.Tensor]):
        ...
        if self._is_mla:
            ...
            self.kv_cahce_ndim = len(kv_caches[0].shape)
            if self.kv_cahce_ndim == 5:
                _, num_blocks, block_size, num_heads, head_dim = kv_caches[0].shape
                self.block_len = block_size * num_heads * head_dim * kv_caches[0].element_size()
            elif self.kv_cahce_ndim == 3:
                # (2, num_blocks, block_size * num_kv_heads * head_size)

                _, num_blocks, block_size = kv_caches[0].shape
                self.block_len = block_size * kv_caches[0].element_size()
            #_, num_blocks, heads = kv_caches[0].shape
            #self.block_len = block_size * num_heads * head_dim * kv_caches[0].element_size()
            ...
            for key_cache, value_cache in kv_caches:
                ...
                region_len = self.num_cache_entries * self.num_blocks * self.block_len
                caches_data.append((base_addr, region_len, self.rank, ""))
    ...
    def read_blocks(self, local_block_ids, staging_block_ids, remote_block_ids, dst_engine_id):
        ...
        kwargs = {}
        if self.kv_cahce_ndim == 3:
            kwargs["block_size"] = self.block_size
            kwargs["num_kv_heads"] = self.num_heads
            kwargs["head_size"] = self.head_size
            ...
            for local_range, staging_range in zip(local_rearranging_ranges, staging_rearranging_ranges):
                ...
                for kv_cache in self.kv_caches:
                    for cache in kv_cache:
                        rearrange_tensors(cache[local_range[0]:local_range[1] + 1], cache[staging_range[0]:staging_range[1] + 1], tp_multiplier, "read", **kwargs)
        ...
        if self._is_mla:
            ...
            for local_range, staging_range in zip(local_rearranging_ranges, staging_rearranging_ranges):
                ...
                for kv_cache in self.kv_caches:
                    for cache in kv_cache:
                        if kwargs:
                            rearrange_tensors(cache[local_range[0]:local_range[1] + 1], cache[staging_range[0]:staging_range[1] + 1], tp_multiplier, "write", **kwargs)
                        else:
