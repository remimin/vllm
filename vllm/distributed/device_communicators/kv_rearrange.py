...
def rearrange_tensors(t1: torch.Tensor, t2: torch.Tensor, d: int, direction: str, **kwargs):
    
    #N, B, H, C = t1.shape
    
    assert t2.shape == t1.shape, "Destination tensor must have same shape as source"
    if "head_size" in kwargs:
        H = kwargs["head_size"]
    if "block_size" in kwargs:
        B = kwargs["block_size"]
    if "num_kv_heads" in kwargs:
        C = kwargs["num_kv_heads"]
    if t1.ndim == 2:
        # (2, num_blocks, block_size * num_kv_heads * head_size)
        N, block_size = t1.shape
    else:
        N, B, H, C = t1.shape
        block_size = None
    ...
    kv_size = N * block_size

    BLOCK_SIZE = 1024
    #grid = ((N * B * H * C + BLOCK_SIZE - 1) // BLOCK_SIZE,)
    grid = ((kv_size + BLOCK_SIZE - 1) // BLOCK_SIZE,)
